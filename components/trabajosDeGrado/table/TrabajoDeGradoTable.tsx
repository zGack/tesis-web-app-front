...
import { TablePagination, TrabajoDeGradoTableDialog } from "@/components/projects";
import TableFilters from "@/components/projects/TableFilters";
import { TrabajoDeGradoTableColumns } from "./TrabajoDeGradoTableColumns";
import { TrabajoDeGrado } from "@/interfaces/trabajoDeGrado";

interface Props {
  trabajosDegrado: TrabajoDeGrado[]
}

const TrabajoGradoTable = ({ trabajosDegrado }: Props) => {

  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
  const [pagination, setPagination] = useState<PaginationState>({pageIndex: 0, pageSize: 2});

  const [isOpen, setIsOpen] = useState(false);
  const [selectedProject, setSelectedProject] = useState<TrabajoDeGrado>(
    trabajosDegrado[0]
  );

  const openModal = (rowData: TrabajoDeGrado) => {
    setIsOpen(true);
    setSelectedProject(rowData);
  }

  const table = useReactTable({
    data: trabajosDegrado,
    columns: TrabajoDeGradoTableColumns,
    initialState: { 
      pagination: { pageSize: 5 } // Numero de trabajos por pagina
    },
    state: {
      columnFilters,
    },
  ...
  return (
    <main className="flex justify-center w-full my-6">
      <div className="flex flex-col overflow-x-auto shadow-md sm:rounded-lg w-full max-w-7xl p-4 min-h-[275px]">
        ...
        <div className="flex items-center space-x-3 py-4 bg-white">
          ...
          <TableFilters 
            ...
            placeholderLabel="Trabajo de grado"
          />
      ...
      {/* Dialog */}
      <TrabajoDeGradoTableDialog isOpen={isOpen} setIsOpen={setIsOpen} project={selectedProject} />
    </main>
  )
}

export default TrabajoGradoTable;
