...
const Register = () => {
  ...
  const onRegister = async ({ email, password, passwordConfirm, name, lastname, personalId }: FormData) => {

    // const logginSuccessful = await loginUser(email, password);

    if ( isNaN(Number(personalId)) ) return;
    ...
    console.log({ email, password, passwordConfirm, name, lastname, personalId });

    const status = await authRegister({email, password, name, lastname, personalId: Number(personalId)}) 
