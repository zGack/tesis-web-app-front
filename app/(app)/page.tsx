...
const menus = [
  {
    ...
    "url": "anteproyectos",
    "rolesAllowed": ['admin', 'director', 'evaluador', 'jurado', 'estudiante'] as ValidRoles[],
    "testTag": "anteproyectos-card"
  },
  {
    "name": "Proyectos de Grado",
    "singularName": "Proyecto de Grado",
    "description": "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Iusto sapiente similique consectetur repudiandae eveniet rem voluptate quibusdam temporibus illo nulla.",
  ...
  },
  {
    ...
    "url": "usuarios",
    "rolesAllowed": ['admin'] as ValidRoles[],
    "testTag": "usuarios-card"
  },
]
...
export default function Home() {
  ...
  const currentUserRoles = session?.user.role;

  return (
    <main className="flex grow justify-center items-center">
      <div className="flex flex-col max-w-screen-md grow space-y-4"> 
        {
          menus.map(({name, singularName, description, url, icon, rolesAllowed, testTag}) => (
            currentUserRoles?.some( role => rolesAllowed.includes(role)) && 
            <Link 
              key={name}
              data-ct={testTag}
              href={url} 
            ...
            >
              ...
              <div className="my-6 mx-5">
                <h2 className="font-semibold text-4xl text-gray-700 mb-4">{ currentUserRoles.includes('estudiante') ? singularName : name}</h2>
                <p>{description}</p>
              </div>
            </Link>
          ))
