...
import { TrabajoDeGradoColumns as columns  } from "./columns";
...
import { TrabajoGradoTable, trabajoDeGradoDataTable } from "@/data";

const TrabajoGrado = () => {
  const [data, setData] = useState([...trabajoDeGradoDataTable]);

  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
  const [pagination, setPagination] = useState<PaginationState>({pageIndex: 0, pageSize: 2});

  const [isOpen, setIsOpen] = useState(false);
  const [selectedProject, setSelectedProject] = useState<TrabajoGradoTable>(data[0]);

  const openModal = (rowData: TrabajoGradoTable) => {
    setIsOpen(true);
